...
public class TablesAnimals extends AbsTables {
    ...
    public List<AbsAnimals> updateAnimalById(int id, List<AbsAnimals> listAnimals) {
        ResultSet rs = getDataById(id);
            ...
            try {
                if (an.getId() == id) {
                    while (rs.next()) {
                        ...
                        an.setWeight(rs.getInt("weight"));
                    }
                }
            } catch (SQLException e) {
                throw new RuntimeException("Не удалось изменить данные животного с id = " + id, e);
            }
        }
        return listAnimals;
    }

    public void updateAnimalTable(int id, String field, String newValue) {
        StringBuilder sb = new StringBuilder();
        String[] getField = field.split(",");
        String[] getNewValue = newValue.split(",");

        if (getField.length != getNewValue.length) {
            throw new IllegalArgumentException("Количество полей и новых значений должно совпадать.");
        }

        for (int i = 0; i < getField.length; i++) {
            sb.append(getField[i] + " = '" + getNewValue[i] + "'");
            if (i != getField.length - 1) {
                sb.append(",");
            }
        }
        update(id, String.valueOf(sb));
    }
